...
public class UserController {
	...
	// id 중복조회
	// POST,PUT,DELETE 으로 전달하면 데이터들이 RequestBody로 전송
	...
	public ResponseEntity<?> authenticate(@RequestBody UserDTO dto) {
		...
		if (user != null) {
			...
			final UserDTO responseUserDTO = UserDTO.builder().userId(user.getUserId()).idx(user.getIdx())
					.name(user.getName()).email(user.getEmail()).token(token).build();
